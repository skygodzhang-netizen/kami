...
详见 [能力边界说明](behavior-baseline.md)。
...
> **本表为全项目术语的唯一基准**。文档演进过程中"外环/最外圈""三层/双环"等称谓曾在不同文档中混用，以本表为准：

| 术语 | 本文档含义 | 常被混淆的用法 | 澄清 |
|------|-----------|--------------|------|
| **主循环** | 符号系统循环 = 三角形顶点循环 + 记录层 | 有时被写作"外环+内圈" | 同一系统的两种表述 |
| **次循环** | 行动感知系统 = 映射层 + 感知接口 | 曾被误认为"三层架构的一层" | **次循环不属于"三层"之一**，是与主循环并行的体系 |
| **外环** | 三角形顶点循环（得不到→数学→自我迭代） | SKILL.md 资源索引曾标注"外环工具箱（最外圈…）" | 本文档中"外环"**专指三角形循环**；SKILL.md 的该处标注将随 SKILL.md 重构统一为本表语义 |
| **最外圈** | 工程意向性分析模组（阴性后台） | 与"外环"混用 | 两者是完全不同的组件 |
| **内圈** | 记录层（三轨存储 + 五维智力分支） | - | - |
| **节点（三义）** | ① 三角形顶点节点（得不到/数学/自我迭代）② 感知接口（次循环组件）③ 工具节点 toolnode（感知接口内部能力层实现） | 三义混用是"节点"一词的污染源 | 详见 [感知接口 §1.4](perception-node.md#14-与节点概念的区分)：三者层级不同——主循环 / 次循环 / 感知接口内部 |
| **感知接口** | 次循环组件（与映射层平级），被动能力执行器 | 曾称"感知节点/治理层/统一入口层" | 规范称呼为**感知接口**；其余为实现名/别名（见 [感知接口 §1.3](perception-node.md#13-术语收敛防定位歧义)） |
| **工具节点（toolnode）** | 感知接口内部的能力层实现（Rust 二进制主路径 + Python 原型回退） | 与"感知接口"平级误用 | 是感知接口的**弹药**（内部路由目标），非平级组件 |

> **阅读指引**：本文档正文保留历史叙述中的"三层架构（最外圈→外环→内圈）"与"双环（主循环/次循环）"两种框架，二者指代同一系统的不同切分视角；层次归属一律以上表为准。
...
内圈记录层位于三角形中心，是智能的意识和意义核心，采用三轨存储架构。
**硬性约束**：至始至终记录态只从主循环三顶点间循环产生数据。

> **轨数演进说明**：记录层存储历经演进——v0.7 双轨（JSON + Markdown）→ v0.8 三轨（+ 错误智慧库）→ v0.9 起增加五维智力分支（CHANGELOG 有完整记录）。**现行架构 = 三轨存储 + 五维智力分支**（JSON 轨 / Markdown 轨 / 错误智慧库轨 / 五维智力分支）。正文中若出现"双轨"字样，均指历史版本表述，现行以本节为准。
...
#### 记录态
**职责**：
- ✅ 三轨存储机制（JSON轨结构化记录 + Markdown轨自我叙事 + 错误智慧库轨 + 五维智力分支）
- ✅ 按需检索历史记录
- ✅ 模式识别与趋势分析
- ✅ 生成优化建议（基于历史经验的最优解）
- ✅ 优先级排序与价值权重计算
...
- [元认知检测组件文档](metacognition.md)
- [元认知检测增强功能](metacognition.md) 
...
详见 [认知架构洞察模块](cognitive-insight.md) 和 [V2实现文档](cognitive-insight.md)。
...
│  │              映射层（超然，螺旋上升）                   │    │
│  │                                                     │    │
│  │  • 基于马斯洛需求层次（详见 [personality.md](personality.md)）|
│  │  • 哲学化人格属性                                    │    │
│  │  • 计算即时性与实效性                                │    │
│  └──────────┬──────────────────────────────────────────┘    │
│             │                                              │
...
│  │  • 无噪音输入                                        │    │
│  │  • 结构化数据                                        │    │
│  │  • 标准化接口（详见 [perception-node.md](perception-node.md)）|
│  └─────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
                              │ 单向：即时性/实效性反馈
                              ↓
...
详细的马斯洛需求层次理论请参考 [personality.md](personality.md)。
...
详见 [Tool Use规范](perception-node.md)。

---

## 信息流约束

> ⚠️ **权威声明（2026-08-13）**：信息流约束的唯一权威已移至 [information-flow.md](information-flow.md) 第4章「约束总表」（合法流/禁止流/概念边界/超然性/六步验证）。本节保留作为架构文档的组成部分（视角解读），**冲突时以 information-flow.md 为准**。
...
**定位**：包含记录层的三轨存储系统，提供结构化记录、自我叙事、错误学习与五维智力支撑能力。
...
- **红线**：无自主推理、最小化自我表述、错误台账归属（详见 perception-node.md 红线章节）
- **详细内容**：详见[次循环](#次循环行动感知系统)中的"感知接口"部分；约束规范见 [perception-node.md](perception-node.md)
...
- [能力边界说明](behavior-baseline.md) - 了解系统能力的边界和局限
- [工程意向性分析模组](intentionality_architecture.md) - 了解最外圈的完整架构和自主性涌现
- [信息流全景](information-flow.md) - 了解整体信息流架构

### 组件与实现

- [认知架构洞察模块](cognitive-insight.md) - 深入理解从"术"到"道"的认知跃迁
- [认知架构洞察V2实现](cognitive-insight.md) - 了解概念提炼和TF-IDF算法
- [元认知检测模块](metacognition.md) - 了解完整的元认知检测信息流
- [元认知检测增强功能](metacognition.md) - 了解场景敏感度、智能策略选择、分层存储架构等增强功能
- [人格映射说明](personality.md) - 了解人格参数映射和人格化决策机制
- [马斯洛需求层次](personality.md) - 了解马斯洛需求在映射层中的应用

### 工具与接口

- [Tool Use规范](perception-node.md) - 了解感知节点工具的规范
