...
# 运行交互式帮助查看器
# python3 scripts/show_help.py（⚠️ 未随技能打包，请用 ci.help() / ci.print_help()）
```
...
**Q4: 如何查看帮助信息？**
A: 使用 `ci.help()` 获取结构化帮助数据，或 `ci.print_help()` 打印帮助信息。（`python3 scripts/show_help.py` 交互式查看器为 V1 时代脚本，未随技能打包）
...
- **帮助系统**：`ci.help()` / `ci.print_help()`（`show_help.py` 未随技能打包）
