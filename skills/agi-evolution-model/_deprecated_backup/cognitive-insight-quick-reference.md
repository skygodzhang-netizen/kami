...
> ⚠️ **注意**：下方 `show_help.py` / `cognitive_insight_help.py` 为 V1 时代帮助脚本，**未随当前技能打包**（scripts/ 目录不存在）。当前请使用 `ci.help()` / `ci.print_help()` 接口。

### 交互式帮助查看器

```bash
cd /workspace/projects/agi-evolution-model/scripts
python3 show_help.py  # ⚠️ 未随技能打包，此处仅为历史示例
```
...
```bash
python3 cognitive_insight_help.py  # ⚠️ 未随技能打包，此处仅为历史示例
```
...
│   ├── cognitive_insight_backup.py       # V1 备份（未随技能打包）
│   ├── concept_extraction_extension.py   # 概念提取扩展
│   ├── cognitive_insight_help.py         # 帮助模块（未随技能打包）
│   └── show_help.py                      # 帮助查看器（未随技能打包）
├── references/
...
- **帮助查看器**: 运行 `python3 show_help.py`（⚠️ 未随技能打包，请用 `ci.help()` 替代）
